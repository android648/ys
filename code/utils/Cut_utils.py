...
def rand_bbox(size, lam=None):
    ...
    cx = np.random.randint(size=[], low=int(W / 8), high=W)
    cy = np.random.randint(size=[], low=int(H / 8), high=H)
    cz = np.random.randint(size=[], low=int(Z / 8), high=Z)
    ...
    # return cx,cy,cz,cut_w,cut_h,cut_z
    return bbx1, bby1, bbz1, bbx2, bby2, bbz2

def cutmix_mask(img, alpha):
    ...
    bbx1, bby1, bbz1, bbx2, bby2, bbz2 = rand_bbox(img.size(), lam=np.random.beta(alpha, alpha))
    ...
    # print(bbx1,bbx2)
    mask[bbx1:bbx2, bby1:bby2, bbz1:bbz2] = 0
    loss_mask[:, bbx1:bbx2, bby1:bby2, bbz1:bbz2] = 0
