...
def pseudo_label(U, eval_net, K):
    ...
    with torch.no_grad():
        Y_u,_,_ = eval_net(U_cap,[])  #the model now is four output, and the teacher network only use the first output
...
def mix_module(X, U, vnet, resnet, K, T, alpha, mixup_mode, aug_factor):
    X_b = len(X)
    U_b = len(U)
    ...
    # step 2: label guessing
    with torch.no_grad():
        Y_u1,_ = vnet(U_cap)  #the model now is four output, and the teacher network only use the first output
        Y_u1 = F.softmax(Y_u1, dim=1)
        Y_u2,_ = resnet(U_cap)  #the model now is four output, and the teacher network only use the first output
