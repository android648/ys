...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_channel=1, width=1,
        ...
        self.layer1 = self._make_layer(block, self.base * 2, layers[0], stride=2)  # layer=3
        self.layer2 = self._make_layer(block, self.base * 4, layers[1], stride=2)  # 4
        self.layer3 = self._make_layer(block, self.base * 8, layers[2], stride=2)  # 6
        if layer4_dilation == 1:
            self.layer4 = self._make_layer(block, self.base * 16, layers[3], stride=2)  # 3
        elif layer4_dilation == 2:
            self.layer4 = self._make_layer(block, self.base * 16, layers[3], stride=1, dilation=2)  # 3
        else:
