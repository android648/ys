...
class SideConv(nn.Module):
    def __init__(self, n_classes=1):
        ...
        self.side5 = nn.Conv3d(1024, n_classes, 1, padding=0)
        self.side4 = nn.Conv3d(512, n_classes, 1, padding=0)
        self.side3 = nn.Conv3d(256, n_classes, 1, padding=0)
        self.side2 = nn.Conv3d(128, n_classes, 1, padding=0)
...
class Resnet34(nn.Module):
    def __init__(self, resnet_encoder=None, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False):
        ...
        self.resnet_encoder = resnet50()

        self.block_five_up = UpsamplingDeconvBlock(n_filters * 64, n_filters * 32, normalization=normalization)

        self.block_six = ConvBlock(3, n_filters * 32, n_filters * 32, normalization=normalization)
        self.block_six_up = UpsamplingDeconvBlock(n_filters * 32, n_filters * 16, normalization=normalization)

        self.block_seven = ConvBlock(3, n_filters * 16, n_filters * 16, normalization=normalization)
        self.block_seven_up = UpsamplingDeconvBlock(n_filters * 16, n_filters * 8, normalization=normalization)

        self.block_eight = ConvBlock(2, n_filters * 8, n_filters * 8, normalization=normalization)
        self.block_eight_up = UpsamplingDeconvBlock(n_filters * 8, n_filters, normalization=normalization)
