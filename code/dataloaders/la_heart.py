...
class LAHeartr(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        train_path = self._base_dir+'/train0.list'
        test_path = self._base_dir+'/test0.list'
...
class Pancras(Dataset):
    ...
    def __init__(self, base_dir=None, base_dir_list = None, split='train',train_flod=None, common_transform=None,sp_transform=None):
        ...
        print(train_flod)
        if split=='train':
            with open(self.base_dir_list+train_flod, 'r') as f:
                self.image_list = f.readlines()
        self.image_list = [item.replace('\n','') for item in self.image_list]
...
class LAHeart(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.common_transform:
            sample = self.common_transform(sample)
        if self.sp_transform:
            # samples = self.sp_transform(sample)
            sample1 = self.sp_transform(sample)
            sample2 = self.sp_transform(sample)
            return [sample1,sample2]
            # return samples
