...
def test_all_case(model_vet,model_resnet, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4,save_result=True, dataset_name="LA", test_save_path=None):
    if dataset_name == "LA":
        with open('/root/autodl-tmp/datasets/LA100/test0.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["/root/autodl-tmp/datasets/LA100/" + item.replace('\n', '') + "/mri_norm2.h5" for item in
                      image_list]
    elif dataset_name == "Pancreas_CT":
        with open('/root/autodl-tmp/datasets/Pancreas/test0.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["/root/autodl-tmp/datasets/Pancreas/" + item.replace('\n', '') for item in image_list]
    elif dataset_name == "Bra":
        with open('/root/autodl-tmp/datasets/BraTS2019/test.txt', 'r') as f:
            image_list = f.readlines()
        image_list = ["/root/autodl-tmp/datasets/BraTS2019/data/" + item.replace('\n', '') + ".h5" for item in image_list]

    loader = tqdm(image_list)
    # total_dice = 0.0
    total_metric = 0.0
    logs = pd.DataFrame(index=[], columns=['i', 'dice', 'JC', 'HD', 'ASD'])
    i = 0
    for image_path in loader:
        id = image_path.split('/')[-1]
        ida = image_path.split('/')[-2]
        h5f = h5py.File(image_path, 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        prediction, score_map = test_single_case_first_output(model_vet,model_resnet, image, stride_xy, stride_z, patch_size,
        ...
        if np.sum(prediction) == 0:
            ...
            dice = single_metric[0]
            JC = single_metric[1]
            HD = single_metric[2]
            ASD = single_metric[3]

            tmp = pd.Series([i, dice, JC, HD, ASD], index=['i', 'dice', 'JC', 'HD', 'ASD'])
            logs = logs._append(tmp, ignore_index=True)  # 将数据输出在excel表中
            logs.to_csv('/root/autodl-tmp/ys/cutmix/Bra_50_VDWC/Brazhu.csv', index=False)
            i += 1
...
def val_all_case(model_name, num_outputs, model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18,
...
def test_single_case_first_output(model_vet, model_resnet, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
    ...
    for x in range(0, sx):
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    y,_ = model_vet(test_patch)
                    if len(y) > 1:
                        y = y[0]
                    y = F.softmax(y, dim=1)
                    z,_ = model_resnet(test_patch)
                    if len(z) > 1:
                        z = z[0]
                    z = F.softmax(z, dim=1)
                y = y.cpu().data.numpy()
